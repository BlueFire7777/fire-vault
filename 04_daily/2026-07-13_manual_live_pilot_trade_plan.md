---
id: FIRE-pilot-D43-trade-plan-2026-07-13
phase: 本番 v0 / W60-pilot-D43 / v1.4.2 朝 pilot 6 日目 / max=50 baseline 初回
priority: 高
status: pre-open、v1.4.2 継続運用 + max=50 baseline 初回稼働
...
# Manual Live Pilot — Trade Plan (2026-07-13 / D43)

## §1 全体方針

**v1.4.2** を正本として 6 日目の朝 pilot. F111 max=50 baseline 初回稼働.

- 100 株標準、非 100 株調整禁止
- entry / watch / excluded 分離
- 9130 demote 効果継続観察 (= excluded 維持、8 日目)
- 4404 v1.4.2 entry 復帰継続 (= ⚠ risk_yen_warning、6 日目)
- 9247 / 9628 連続 12 日目 (= 固定化リスク継続、単独 demote 候補 D44+ HQ approve 後)
- **max=50 baseline で entry 14 件**、朝判断は **top 5 優先** + full list 参考

## §2 D43 F111 出力 (= v1.4.2 + max=50 baseline)

- input: `/tmp/fire_d43_prep/d43_f111.json`
- cli_version: **1.3.0** ✓ (= W60-F111-baseline-50)
- max_candidates: **50** ✓ (= baseline 化後の自然適用)
...
## §3 9130 demote 効果継続 (8 日目) + 4404 v1.4.2 継続 (6 日目)

### 3.1 9130 (= demote 効果継続 8 日目: D36-D43)
...
| 連続 excluded | **8 日連続** (= D36 を 1 とする counter) |

### 3.2 4404 (= v1.4.2 entry 復帰 6 日目)
...
| 連続 entry | **6 日連続** (= D38-D43) |

### 3.3 9247 / 9628 固定化リスク (= 連続 12 日目)

| code | 連続 entry 日数 | 固定化リスク |
|---|---|---|
| 9247 | **12 日連続** (D32-D43) | 9130 閾値 (5 連続) の 2.4 倍超 |
| 9628 | **12 日連続** (D32-D43) | 同上 |

→ D44 以降の単独 demote 検討 (= HQ approve 後の別 wave、D41 sim A or B 採用).

## §4 D43 朝の優先確認 top 5 (= max=50 baseline 朝判断用)

### 4.1 [top 1] 9247 ＴＲＥ HD [連続 12 日目]
...
| 見送り条件 | gap > 1,644 / 板薄 / spread 拡大 / 9628 同 sector 重複 / 連続 12 日固定化注意 |

### 4.2 [top 2] 9628 燦 HD [連続 12 日目]
...
| 見送り条件 | gap > 1,418 / 板薄 / spread 拡大 / 9247 同 sector 重複 / 連続 12 日固定化注意 |

### 4.3 [top 3] 4404 ミヨシ油脂 [⚠ risk warning、連続 6 日目]
...
### 4.4 [top 4] 3089 テクノアルファ [新規 / 商社 sector 多様化]
...
| 株数 | 100 株 |
| risk_yen | 5,275 円 |
| sector | 商社・卸売 (= sector 多様化新規) |
| 見送り条件 | gap > 1,076 / 板薄 / spread 拡大 / 出来高薄注意 (新規入口、流動性慎重確認) |

### 4.5 [top 5] 9633 東京テアトル [新規 / 不動産 sector 多様化]
...
| 株数 | 100 株 |
| risk_yen | 7,895 円 |
| sector | 不動産 (= sector 多様化新規) |
| 見送り条件 | gap > 1,610 / 板薄 / spread 拡大 / 出来高薄注意 (新規入口) |

## §5 参考: top 5 以外の entry 9 件 (= max=50 baseline 効果)

| # | code | 銘柄 | sector | 価格 | 100 株 risk_yen | warning |
|---|---|---|---|---|---|---|
...
朝判断時間が許す場合のみ参考確認、または D44 以降検討.

## §6 excluded 21 件

- demote 8 件: 9130 (D36 demote、8 日目) / 8747 / 5729 / 3489 / 340A0 / 3798 / 137A0 / 7991
...
→ D38-D42 baseline (max=20) の 2 sector から **7 sector** に拡張 (= +5)
→ max=50 baseline 化の効果確認 ✓

注: F111 出力上の sector_17_name は 12 種だが entry 上は 7 種 (= 残り 5 種は excluded のみ).
...
### 9.1 pre-open
1. F111 D43 確認 (= max=50 baseline 初回)、consumer payload 確認、Markdown 確認
2. top 5 advisory 確認 (= 主役 3 + sector 多様化新規 2)
3. iSPEED で top 5 確認
4. Fujiwara 最終判断:
   - 9247 / 9628: entry 検討、ただし連続 12 日目 + 同 sector 重複慎重
   - 4404: entry 検討可、risk 警告許容判断
   - 3089 / 9633: 新規 entry、流動性慎重確認
   - 9130: skip 確定

### 9.2 寄り後 / 前場後
1. actual_flow_d43.json 作成 (= template コピー)
...
2. v1.4.2 6 日目運用検証
3. 9130 demote 8 日目確認
4. 9247/9628 連続日数更新 (= D44 で 13 日 or リセット)
5. 新規 (3089/9633) の entry 実績評価
...
- 生成 file: `/tmp/fire_d43_prep/` + vault `~/fire-vault/04_daily/2026-07-13_*.md`
- 9247/9628 demote 本実行は別 wave (= HQ approve 後)
- F111 max=50 baseline 初回、max=100/200/300 は採用せず
...
- F111-THEME-SECTOR-OVERLAY-R1 設計 wave 起票検討
