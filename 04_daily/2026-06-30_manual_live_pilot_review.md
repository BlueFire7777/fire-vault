---
id: FIRE-pilot-D34-review-2026-06-30
phase: 本番 v0 / W60-pilot-D34 / v1.4.1 review template (= 10 項目構造)
priority: 高
status: pending Fujiwara input (= D34 close 後に埋める)
owner: BlueFire7777 (Fujiwara)
date: 2026-06-30 (= D34、火)
parent_trade_plan: 04_daily/2026-06-30_manual_live_pilot_trade_plan.md
---

# Manual Live Pilot — Trade Review (2026-06-30 / D34)

> 本 review は D34 close 後に Fujiwara が手動で埋める。OHLCV / signal_success / entry_success は
> Fujiwara input required (= 推測禁止)。
>
> 朝 advisory 推奨 4 候補 (= 9130 / 9247 / 9628 / 4404) のみを対象。
> 9130 は no-new-chase 既定 (= **D34 連続 4 日目**)、D35 で 5 連続なら demote simulation 候補.

## §0 D34 actual status snapshot (= pending Fujiwara input)
...
- **9247 result**: pending
- **9628 result**: pending
- **9130 no-new-chase observed**: Fujiwara input required (= pre-open override で skip 確定済、
...
- **§1-§10 review items**: 各 [必須 N] Fujiwara input required を保持

→ 本 wave は **pilot pre-open 準備** のみ、actual fill は D34 close 後の別 wave で実施.
...
| code | 判断理由 |
|---|---|
| 9130 | Fujiwara input required (= 既定理由: no-new-chase / D34 連続 4 日目 / D35 = 5 連続 warning + demote simulation 候補 / D36 = 必要なら demote 本実行候補) |
| 9247 | Fujiwara input required |
| 9628 | Fujiwara input required |
| 4404 | Fujiwara input required (= 既定理由: 100 株 risk_yen 上限超 / 非 100 株調整不採用) |

## §3 [必須 3] 実際に見た銘柄 (= D34 朝 advisory 推奨 4 件のみ)

- **9130 共栄タンカー** (= 海運業 / タンカー、no-new-chase、**連続 4 日目**)
- **9247 ＴＲＥ HD** (= サービス業 / リサイクル、entry_candidate #1)
- **9628 燦 HD** (= サービス業 / 葬祭、entry_candidate #2)
- **4404 ミヨシ油脂** (= 食品工業 / 油脂、watch 降格、entry 不可)

D31 (= 9130/331A0/4389) や D32 (= 9247/9628/9130/4404) や D33 candidate と
**混同しない**. D34 review は **D34 朝 advisory 推奨 4 件のみ** が対象.

## §4 [必須 4] 板・出来高・spread + 前場/終日実績

| code | 寄値 | 高値 | 安値 | 終値 | 前日終値比 | 出来高 | spread 所感 | 当日方向感 |
|---|---|---|---|---|---|---|---|---|
| 9130 | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required | (= 観察のみ、no-new-chase 既定 / 連続 4 日目) |
| 9247 | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required | Fujiwara input required (= D32/D33 で出来高薄 / spread 大の懸念あり) | Fujiwara input required |
...
Fujiwara input required (= D34 close 後)。

参考観点:
- 銘柄選定 signal の妥当性 (= F111 v1.4.1 出力が合っていたか)
- 9130 連続 4 日目の signal 観察 (= chase_limit 超過観測 / 過熱判定)
- 9247 / 9628 の出来高 / spread / 約定リスク
- 4404 100 株 risk 上限超で entry 不可の妥当性
- **D35 demote simulation 候補判定** (= 9130 が D35 で再び entry 上位なら sim 実行)

## §6 [必須 6] 実 entry か skip か (= D34 actual、100 株標準)

| code | 判定 |
|---|---|
| 9130 | **NO enter (= no-new-chase 既定 / 連続 4 日目)** ✓ 既定 + Fujiwara 確認 input pending |
...
| code | 価格 or 理由 |
|---|---|
| 9130 | 見送り (= no-new-chase、D34 連続 4 日目、D35 = 5 連続 warning + demote simulation 候補 / D36 = 必要なら demote 本実行候補) |
| 9247 | Fujiwara input required |
| 9628 | Fujiwara input required |
...
| code | 判断 phase |
|---|---|
| 9130 | **pre-open 判断** (= 朝 advisory 段階で no-new-chase 確定、D34 連続 4 日目で過熱判定) |
...
| 9247 | Fujiwara input required (= 当日 +2R take_profit 1,773 到達したか) | Fujiwara input required |
...
- **historical review** (= 本 file): 当日朝 advisory で出した利確目安 / 追わない上限 / 損切り目安に
  対する事後評価。Fujiwara が D34 close 後に手動記録。
- **current renderer** (= compute_signal_success): post-open observation の high / low が candidate の
  +2R take_profit / -1R stop_loss を hit したかで自動判定。obs 未提供 / partial → pending。推測禁止。

→ 両者は **別 field / 別意味** として記録。current renderer 値で historical review を
**上書きしない**.

## §11 D31/D32/D33 からの引き継ぎ / 9130 連続観察 (= demote 日程補正版)
...
- D33 (= 2026-06-29): 9130 entry_candidate → no-new-chase 維持、**連続 3 日目**
- D34 (= 2026-06-30): 9130 entry_candidate → **no-new-chase 維持**、**連続 4 日目** (= 本日)
- **D35 (= 2026-07-01 予定): 5 連続確認 → 5 連続 warning + demote simulation 候補** (= 9130 を
  recently_seen_codes に追加した F111 を別途実行し、role 変化 / 候補 churn を確認)
...
**出る限り**カウントを継続. 1 日でも entry 上位から外れたら counter リセット.

## §12 D34 actual_flow input (= post-open wrapper 用)

template: `/tmp/fire_d34_prep/d34_actual_flow_template.json`

Fujiwara が D34 場中・前場後に手動入力する用の placeholder. 推測禁止、未観測は null のまま.
...
- F282 production plist と smoke plist と LaunchAgents plist を別 file として safety final 確認
- 100 株標準 / 非 100 株調整 0 / forbidden phrase 0
- D31/D32/D33/D34 を fixture / docs で分離維持
