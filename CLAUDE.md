...
## タスク完了基準 (2026-05-04 v1.0 制定 → 2026-05-05 v1.2 / F271 改訂)

タスクの完了報告では **「動いた / 機能した / 期待値を満たした」の 3 段階** を
区別すること。

1. **動いた** — exit code 0 / ファイル作成成功
2. **機能した** — 意味的成功条件 (events>0 / Wikilink 健全性 / リンク先存在)
3. **期待値を満たした** — 受入基準達成 (件数 / カバレッジ / 性能 / Fujiwara
   判定)

「動いた」だけで完了報告するのは原則禁止 (軽微タスクを除く)。
完了報告時は到達段階を明示し、「機能した」「期待値達成」の根拠を検証コマンド
出力等で示す。

- **最新版正本**: [[03_design/F271_v1.2|F271 v1.2 (2026-05-05、F278 Phase 5 リリース)]]
  (§6 アンチパターン §6-1〜§6-16 / §3.6 パターンライブラリ §3.6.1〜§3.6.14 /
   事前 grep 標準観点 v3 7 観点)
- **ルール正本**: [[タスク運用ルール]] § 8 タスク完了基準 + § 9 TODO Excel
  Vault 同期戦略
- **詳細仕様書**: [[03_design/task_completion_criteria|タスク完了基準仕様書]]
  (3 段階定義 / 種別ガイドライン / 報告テンプレ / アンチパターン集、v1.0 仕様)

### 重要原則 (Claude が常に意識すべき)

- **exit code 0 = 成功 ではない**
- **「将来作るので OK」は完了ではない**
- **テスト PASS だけで「機能した」と判定しない** (smoke 必要)
