...
# F062-R5.8: Action-First Advisory Production Send
...
.venv/bin/python -m scripts.jobs.run_f062_research_advisory_line_preview \
  ...
  --gate-json    /tmp/f062_r5_8_gate.json \
  --listings-db  data/fire.staging.db \
  --message-mode production --action-mode --card-top 5 \
  --output-json    /tmp/f062_r5_8_line_payload.json \
  --output-text    /tmp/f062_r5_8_line_preview.txt \
  --output-summary-json /tmp/f062_r5_8_line_summary.json
```
...
| metadata.name_enrichment.enriched  | 30 / 30 |
| chunks                             | 1 |
| chunk[0] length                    | **738** ★ (= R5.6 1,120 から 34% 短縮) |
| forbidden_phrase_count             | 0 |
| safety_footer_present              | True |
| selected_count                     | 5 |
| selected_label_counts              | boost_with_avoid: 5 |

**action bucket counts (= Top N selected ベース、画面と完全整合):**
| bucket | count |
|---|---|
| 今すぐ買い                  | 0 |
| 条件付き買い                | 0 |
...
`run_f062_line_production_send_smoke ... --max-chunks 1` (`--send` なし)
...
| token_read_count           | 0 |
| production_callable_built  | False (= --send なし、callable 未構築) |
...
source ~/.fire_secrets/line.env && .venv/bin/python -m \
  ...
  --max-chunks 1 \
  --output-json       /tmp/f062_r5_8_real_send.json \
  --completion-report /tmp/f062_r5_8_real_send_report.txt
```
...
| production_outcomes        | 1 件 (label=f062_r3_smoke / chunk_index=0 / **send_text_status=ok** ★ / chunk_length=738 / recipient_type=user / recipient_hash8=b344b213) |
...
```
2026-05-11T09:19:53  SEND  user:prefix=U:len=33:hash8=b344b213
...
| F119 内部 key (snake_case) の LINE 漏洩                         | 0 ✓ |
...
- 送信時刻: **2026-05-11T09:19:53 UTC** (= 2026-05-11 18:19 JST)
- 送信件数: **1 件のみ** (= 重複なし)
- chunk 内容: action-mode 行動判断カード、length=738
...
買いに変わる条件: VWAP 上維持 + 出来高増   ★ 次のアクション

🟠 待ち 340A0 ジグザグ
判断: 待ち
理由: 業種×月の条件は良いが、上位rank×業種条件に弱さあり
買いに変わる条件: VWAP 上維持 + 出来高増

🟠 待ち 37980 ＵＬＳグループ ... (Top 5、各 4 行)
🟠 待ち 137A0 Ｃｏｃｏｌｉｖｅ ...
🟠 待ち 331A0 メディックス ...
...
「今日の結論: 待ち」 + Top 5 「判断 / 理由 / 買いに変わる条件」の
行動判断のしやすさ / 重複なし をご報告ください。
...
| 2026-05-11 17:02 (F062-R5.6)  | buyability + names            | 1,120 |
| **2026-05-11 18:19 (F062-R5.8)** | **action-first (画面整合)** ★ | **738** |

## F062-R5.6 → R5.8 の文面進化 (本送信を Fujiwara 視点で比較)

**F062-R5.6** (= buyability、結論と Top 表示が乖離):
```
🟠 結論: 買い検討候補あり。ただし慎重寄り
買い検討 30 / 注意 0 / 見送り 0          ← 入力全体ベース
🟠 強弱混在・慎重 1. 57290 日本精鉱
判定: 強弱混在・慎重
理由: F119 boost month_of_year / avoid sector_17 一部該当 ★内部 key
```

**F062-R5.8** (= action-first、画面整合 + 内部 key 除去):
```
🟠 今日の結論: 待ち                       ← Top 5 と完全整合
今すぐ買い: 0件 / 待ち: 5件
🟠 待ち 57290 日本精鉱
判断: 待ち
理由: 月の条件は良いが、業種条件に弱さあり  ★ 自然言語
...
enrichment は影響なし。advisory_preview の再生成のみが必要。
...
1. ★ F062-R5.8 完了 (= 本書、行動判断カード 1 chunk 送信成功)
2. **Fujiwara が LINE app で 1 通受信確認** → 「今日の結論」と Top 5
   「判断 / 理由 / 買いに変わる条件」のレビュー
3. 受信成功確認後:
