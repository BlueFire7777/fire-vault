...
priority: 高 (Phase F287 仕様 v1.1 完了、F288 着手前)
status: 仕様設計 + feasibility 確認完了 (v1.1、HQ 補正反映: 主案 = Claude API、Plugin = 半自動補助、F290 必須検証 8 項目)
owner: Fujiwara
...
## feasibility 確認 結論 (v1.1、HQ 補正反映、詳細は仕様書 §14)

★ **主案: Claude API + PDF 入力 + 構造化 JSON 出力** ★

- 全銘柄を Claude API + PDF 入力で自動処理 (anthropic SDK)
- tool use で 16 観点 JSON schema を強制、根拠箇所も出力
- PDF 内に根拠がない項目は **明示的に "unknown"** を返す (捏造禁止)
- スライドは Markdown → PDF が現実的、PPT は python-pptx で代替

★ **Plugin 系は半自動補助案として残す (HQ v1.1 補正)** ★

- financial-services plugin / Claude.ai plugin / Claude for PowerPoint
  は **直接自動パイプラインに組み込めると断定しない**
- 高重要度銘柄 (importance 5 = 保有 + Research A1 等) の **半自動補助**
  として残す: Fujiwara が Claude.ai web で同一 PDF を分析、結果を
  所定フォーマットで FIRE に貼付け (Plugin 出力フォーマットは要 Phase
  F290 以降の実機検証、自動経路には組み込まない)
...
## F290 実装時の必須検証 8 項目 (HQ v1.1 補正)

仕様書 §12 F290 受入基準を参照。実装着手前 / 着手中に以下 8 項目を
確実に検証する:

1. **ANTHROPIC_API_KEY 権限確認** (Anthropic Console で Fujiwara が確認)
2. **1 銘柄 1 短信 PDF で API 疎通** (smoke test、200 OK 取得)
3. **コスト実測** (token 消費 × Claude pricing で月額試算、Fujiwara 判断)
4. **16 観点 JSON schema 固定** (tool use input schema 定義、出力 validation)
5. **根拠箇所保存** (PDF page / section を JSON 内に含める)
6. **PDF 内根拠がない項目は unknown** (捏造禁止、null/0 で埋めない)
7. **Fujiwara レビュー前提** (投資判断系は draft/ → 承認後 approved/)
8. **自動発注 / 楽天証券操作には接続しない** (R-03-01 / R-13-08 厳守)
